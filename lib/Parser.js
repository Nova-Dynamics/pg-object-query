...
module.exports = class Parser extends EventEmitter {
    ...
    parse(str) {
        ...
        if ( !this.tokenizer.EOF ) {
            throw new SyntaxError(`Expected characters near: ${this.tokenizer.cursor}`);
        }
    ...
    parse_fragment_list() {
        ...
        while ( !this.tokenizer.EOF && this.tokenizer.next_token.type != "}") {
    ...
    parse_fragment() {
        switch ( this.tokenizer.next_token.type ) {
            ...
            case "(&)[": {
                this.eat("(&)[")
                const obj = {
                    type: "AndDelimitedArray",
                    value: this.parse_variable_list(" AND ")
                }
                this.eat("]");
                return obj;
            }
            case "(|)[": {
                this.eat("(|)[")
                const obj = {
                    type: "OrDelimitedArray",
                    value: this.parse_variable_list(" OR ")
                }
                this.eat("]");
                return obj;
            }
            case "(,)[": {
                this.eat("(,)[")
                const obj = {
                    type: "CommaDelimitedArray",
                    value: this.parse_variable_list(", ")
                }
                this.eat("]");
                return obj;
            }
